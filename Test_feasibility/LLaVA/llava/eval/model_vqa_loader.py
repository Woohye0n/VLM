...
def eval_model(args):
    ...
    if positional_embedding_type == "default":
        ...
        elif positional_embedding_type == "reduced_interval":
            original_embedding = model.model.vision_tower.vision_tower.vision_model.embeddings.position_embedding.weight.data
            original_num_patches = original_embedding.size(0)-1  
            step_interval = (original_num_patches) // (num_patches)
            
            # Keep the first position, then sample the rest at intervals
            cls_position = original_embedding[0].unsqueeze(0)  # First position for CLS
            sampled_positions = original_embedding[1:][::step_interval][:num_positions - 1]  # Intervals applied from position 2 onward
            
            # Concatenate CLS with sampled positions
            final_positions = torch.cat((cls_position, sampled_positions), dim=0)
            
            # Update the position embedding with sampled positions
            model.model.vision_tower.vision_tower.vision_model.embeddings.position_embedding = nn.Embedding(num_positions, embed_dim).to(device)
            model.model.vision_tower.vision_tower.vision_model.embeddings.position_embedding.weight.data.copy_(final_positions)
