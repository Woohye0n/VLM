...
class LlavaMetaModel:
    ...
    def get_downsampled_vision_tower(self):
        downsampled_vision_tower = getattr(self, "downsampled_vision_tower", None)
        if type(downsampled_vision_tower) is list:
            downsampled_vision_tower = downsampled_vision_tower[0]
...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_downsampled_vision_tower(self):
        return self.get_model().get_downsampled_vision_tower()
    ...
    def encode_downsampled_images(self, images):
        image_features = self.get_model().get_downsampled_vision_tower()(images)
        # image_features = self.get_model().vision_resampler(image_features, images=images)
        image_features = self.get_model().mm_projector(image_features)
        return image_features
    ...
    labels, images, downsampled_image=None, divide_shape=None, modalities=["image"], image_sizes=None, image_mask=None, generation_type="default"):
        ...
        if type(images) is list or images.ndim == 5:
            ...
            if downsampled_image is not None:
                encoded_downsampled_image_features = self.encode_downsampled_images(downsampled_image.squeeze(0))
            # image_features,all_faster_video_features = self.encode_multimodals(concat_images, video_idx_in_batch, split_sizes)
            ...
            if mm_patch_merge_type == "flat":
                ...
                for image_idx, image_feature in enumerate(image_features):
                    ...
                    if image_idx in video_idx_in_batch:  # video operations
                        ...
                        if generation_type == "downsampled" or generation_type == "recursion_stage1":
                        ...
                        if "maxpool2x2" in mm_patch_merge_type:
                            ...
                            if image_mask is None:
                                recursed_features = base_image_feature
                            else:
                                base_image_feature = base_image_feature.view(height, width, -1)
                                for row, col in image_mask:                                    
                                    temp = base_image_feature[row*2:row*2+2, col*2:col*2+2, ...]
                                    temp = temp.flatten(0, 1)
                                    recursed_features.append(temp)
                        elif generation_type == "downsampled_tot":
                            base_image_feature = torch.cat((downsampled_base_image_feature, base_image_feature), dim=0)
                            image_feature = image_feature.permute(4, 0, 2, 1, 3).contiguous()
                            image_feature = image_feature.flatten(1, 2).flatten(2, 3)
                            image_feature = image_feature.flatten(1, 2).transpose(0, 1)
                        ...
                        if "nobase" in mm_patch_merge_type:
                            ...
                            if generation_type == "downsampled":
                                image_feature = downsampled_base_image_feature
                            elif generation_type == "recursion_stage1":
                                if type(recursed_features) is list:
                                    recursed_features = torch.cat(recursed_features)
                                image_feature = torch.cat((downsampled_base_image_feature, recursed_features), dim=0)
                            elif generation_type == "downsampled_tot":
                                image_feature = torch.cat((base_image_feature, image_feature), dim=0)
                            elif image_mask is not None and type(recursed_features) is list:
        ...
        _input_ids = input_ids
        input_ids = [cur_input_ids[cur_attention_mask] for cur_input_ids, cur_attention_mask in zip(input_ids, attention_mask)]
        #print(f"input_ids_1: {input_ids[0].shape}")
        labels = [cur_labels[cur_attention_mask] for cur_labels, cur_attention_mask in zip(labels, attention_mask)]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            cur_labels = labels[batch_idx]
            cur_labels_noim = []
            #print(f"image_token_indices_length: {len(image_token_indices)}")
            #print(f"current_input_ids: {cur_input_ids.shape}")
            for i in range(len(image_token_indices) - 1):
                cur_input_ids_noim.append(cur_input_ids[image_token_indices[i] + 1 : image_token_indices[i + 1]])
                cur_labels_noim.append(cur_labels[image_token_indices[i] + 1 : image_token_indices[i + 1]])
            split_sizes = [x.shape[0] for x in cur_labels_noim]
            #print(f"cur_input_ids_noim: {torch.cat(cur_input_ids_noim).shape}")
            cur_input_embeds = self.get_model().embed_tokens(torch.cat(cur_input_ids_noim))
            cur_input_embeds_no_im = torch.split(cur_input_embeds, split_sizes, dim=0)
            cur_new_input_embeds = []
            cur_new_labels = []
        ...
        #print(f"new_input_embed: {new_input_embeds[0].shape}")
        ...
        new_input_embeds = torch.stack(new_input_embeds_padded, dim=0)
        #print(f"new_input_embed_1: {new_input_embeds[0].shape}")
        # rank0_print("tokenizer padding")
