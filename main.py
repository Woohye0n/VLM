...
for idx, i in enumerate(range(len(dataset))):
    ...
    for obj in target:
        ...
        get_value = mask_dict.get(category_name)
        if get_value is not None:
            area, category_mask = get_value
            category_mask = category_mask | mask
        else:
            category_mask = mask
        
        mask_dict[category_name] = (category_mask.sum(), category_mask)
    
    # Create sorted list by mask area
    category_list = sorted(mask_dict, key=lambda x: mask_dict[x][0], reverse=True)
