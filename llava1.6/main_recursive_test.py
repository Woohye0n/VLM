...
def run():
    ...
    for img_idx, i in enumerate(range(len(dataset))):
        ...
        for cat_idx, cat_name in enumerate(predict_list):
            ...
            print(f"attention: {outputs.attentions[0][0].shape}")
            print(f"len_attn: {len(outputs.attentions)}")
            heat_torch_stack, img_with_attn, ret_attn = get_heatmap(model, outputs, tokenizer, prompt, image, input_ids, None)
            print(text)
        ...
        break
            ...
            # is_positive = cat_idx < k
            # hallucination = (is_positive and answer == "no") or (not is_positive and answer == "yes")
            # with open(f"./recurse01.csv", "a") as f:
