...
def get_heatmap(model, outputs, tokenizer, prompt, image, input_ids):
    # constructing the llm attention matrix
    aggregated_prompt_attention = []
    for i, layer in enumerate(outputs["attentions"][0]):
        layer_attns = layer.squeeze(0)
        # print(layer.shape, layer_attns.shape)
        attns_per_head = layer_attns.mean(dim=0)
    ...
    input_token_len = model.get_vision_tower().num_patches * 5 + len(input_ids[0]) - 1 # -1 for the <image> token
    ...
    print(output_token_start, output_token_end)
    print(f"outputs len: {len(output_token_inds)}")
    heat_torch_stack = []
    ...
    return heat_torch_stack, img_with_attn