...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        images, image_sizes=None, image_mask=None
    ):
                    ...
                    if image_feature.shape[0] > 1:
                        ...
                        if 'unpad' in mm_patch_merge_type:
                            ...
                            if image_mask is not None:
                                # print(f"bp1 {image_feature.shape}")
                                image_feature = image_feature.permute(0, 2, 1, 3, 4).contiguous()
                                # image_feature = image_feature.view(4, height, width, -1)
                                # print(f"bp2 {image_feature.shape}")
                                image_feature = image_feature.view(height * 2, width * 2, -1)

                                ####재익#########
                                output_features = torch.zeros((24 * 24 + len(image_mask) * 3, image_feature.shape[-1]))
                                print(f'output_features.shape: {output_features.shape}')
                                image_mask = {tuple(mask.tolist()) for mask in image_mask}
                                index = 0
                                for row in range(24):
                                    for col in range(24):
                                        if (row, col) in image_mask:
                                            temp = image_feature[row*2:row*2+2, col*2:col*2+2, ...]
                                            temp = temp.flatten(0, 1)
                                            output_features[index:index+4, :] = temp
                                            index += 4  # 2x2 패치이므로 인덱스 4만큼 이동
                                        else:
                                            output_features[index, :] = image_feature[row, col, ...]
                                            index += 1  # 1x1 패치이므로 인덱스 1만큼 이동

                                ####재익#########

                                # for row, col in image_mask:
                                #     # print(f"bp5 {row}, {col}")
                                #     # print(f"bp3 {image_feature.shape}")
                                #     temp = image_feature[row*2:row*2+2, col*2:col*2+2, ...]
                                #     temp = temp.flatten(0, 1)
