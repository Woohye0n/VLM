...
def get_heatmap(model, outputs, tokenizer, prompt, image, input_ids, folder=None):
    ...
    #     + list(map(aggregate_llm_attention, outputs["attentions"][1:]))
    # )
    llm_attn_matrix = heterogenous_stack(
        [torch.tensor([1])]
        + list(aggregated_prompt_attention) 
    ...
    # print(f'input_token_len.shape: {input_token_len}')
    ...
    # print("!!!len1!!!", aggregated_prompt_attention.shape)
    # print("!!!len1!!!", llm_attn_matrix.shape)
    # print("!!!len1!!!", len(input_ids[0]))
    # print("!!!len1!!!", len(overall_attn_weights_over_vis_tokens))
    # ax.plot(overall_attn_weights_over_vis_tokens)
    # print("!!!len2!!!", len(outputs["sequences"][0].tolist()))
...
def temp_vis_attn(model, outputs, tokenizer, prompt, image, input_ids):
    image_attentions = []
    for layer in model.get_vision_tower().image_attentions:
        layer = layer[0, ...].unsqueeze(0)
        image_attentions.append(layer)
    ...
    vis_list = []
    for vis_attn in vis_attn_matrix:
        vis_attn = vis_attn.reshape(24, 24)
        vis_list.append(vis_attn)
    vis_attn = torch.stack(vis_list).sum(dim=0)
    vis_attn = vis_attn / vis_attn.max()
    
    attn_over_image = vis_attn.to(torch.float32)
    attn_over_image = F.interpolate(
            attn_over_image.unsqueeze(0).unsqueeze(0), 
            size=image.size, 
            # mode='nearest', 
