...
def run():
    ...
    for img_idx, i in enumerate(range(len(dataset))):
        ...
        for cat_idx, cat_name in enumerate(predict_list):
            ...
            print(f"\n{prompt}\n")
            print("input ids: ", len(input_ids[0]))
            ...
            # img_with_attn = temp_vis_attn(model, outputs, tokenizer, prompt, image, input_ids)
            ...
            # cv2.imwrite(f"./img/{img_idx}_{cat_idx}/{hallucination}_{i}.png", img_with_attn)
            continue
